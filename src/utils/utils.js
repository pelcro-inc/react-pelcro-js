...
/**
 * Utility function to track Google login debugging information
 * @param {string} event - The event name
 * @param {Object} data - Additional data to track
 */
export const trackGoogleLoginEvent = (event, data = {}) => {
  notifyBugsnag(() => {
    Bugsnag.leaveBreadcrumb(`react-pelcro-js: GoogleLogin - ${event}`, {
      ...data,
      ...
      hasGoogleAuth: typeof gapi?.auth2 !== 'undefined',
...
/**
 * Utility function to check if popup blockers are active
 * @returns {Object} Popup blocker detection results
 */
export const detectPopupBlockers = () => {
  const testPopup = window.open('', '_blank', 'width=1,height=1');
  const isBlocked = !testPopup || testPopup.closed || typeof testPopup.closed === 'undefined';
...
/**
 * Utility function to get comprehensive Google login diagnostic information
 * @returns {Object} Diagnostic information
 */
export const getGoogleLoginDiagnostics = () => {
  const diagnostics = {
    source: 'react-pelcro-js',
    component: 'GoogleLoginButton',
    timestamp: new Date().toISOString(),
    ...
    hasGapi: typeof gapi !== 'undefined',
    hasGoogleAuth: typeof gapi?.auth2 !== 'undefined',
    gapiLoaded: typeof gapi?.load === 'function',
    auth2Initialized: typeof gapi?.auth2?.init === 'function',
    popupBlocked: detectPopupBlockers().isBlocked,
    site: window.Pelcro?.site?.read(),
    user: window.Pelcro?.user?.read(),
    environment: window.Pelcro?.environment,
    uiVersion: window.Pelcro?.uiSettings?.uiVersion,
    googleClientId: window.Pelcro?.site?.read()?.google_app_id ? 'present' : 'missing',
  ...
  // Add GAPI specific diagnostics if available
  if (typeof gapi !== 'undefined') {
    try {
      diagnostics.gapiAuthInstance = gapi.auth2?.getAuthInstance() ? 'available' : 'not_available';
      diagnostics.isSignedIn = gapi.auth2?.getAuthInstance()?.isSignedIn?.get() || false;
    } catch (error) {
      diagnostics.gapiError = error.message;
    }
  }
